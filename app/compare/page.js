...
import {
    ...
    HiOutlineStar, HiOutlineChevronDown, HiOutlineDocumentArrowDown, HiOutlineScissors
        ...
        <div>
                    ...
                    <div style={{ display: 'flex', gap: '12px' }}>
                        ...
                        {data && (
                            <button className="btn btn-secondary" onClick={() => {
                                const doc = new jsPDF();
                                doc.setFontSize(20);
                                doc.text("Executive Quotation Report", 14, 22);
                                doc.setFontSize(11);
                                doc.text(`Generated by Nexus Prime AI on ${new Date().toLocaleDateString()}`, 14, 30);

                                doc.setFontSize(14);
                                doc.text("Overall Recommendation", 14, 45);
                                doc.setFontSize(10);
                                const splitText = doc.splitTextToSize(comparison?.recommendation_justification || "", 180);
                                doc.text(splitText, 14, 52);

                                const tableData = vendors.map(v => [
                                    v.document_metadata.vendor_name,
                                    `${v.mcd_scoring.nexus_trust_score}/100`,
                                    `$${v.commercial_summary.true_total_landed_cost_usd?.toLocaleString()}`,
                                    `${v.commercial_summary.normalized_delivery_days} days`,
                                    v.risk_analysis.overall_risk_level
                                ]);

                                doc.autoTable({
                                    startY: 70,
                                    head: [['Vendor', 'Nexus Score', 'Landed Cost', 'Delivery', 'Risk Level']],
                                    body: tableData,
                                    theme: 'grid',
                                    headStyles: { fillColor: [108, 99, 255] }
                                });

                                if (comparison?.split_award_optimization?.status === "RECOMMENDED") {
                                    const finalY = doc.previousAutoTable.finalY || 70;
                                    doc.setFontSize(14);
                                    doc.text("Split-Award Optimization", 14, finalY + 15);
                                    doc.setFontSize(10);
                                    doc.text(`Potential additional savings: $${comparison.split_award_optimization.potential_savings_usd.toLocaleString()}`, 14, finalY + 22);

                                    const splitData = comparison.split_award_optimization.line_item_assignments.map(itm => [
                                        itm.description,
                                        itm.best_vendor,
                                        itm.quantity.toString(),
                                        `$${itm.subtotal.toLocaleString()}`
                                    ]);

                                    doc.autoTable({
                                        startY: finalY + 28,
                                        head: [['Item Description', 'Recommended Vendor', 'Qty', 'Subtotal']],
                                        body: splitData,
                                        theme: 'grid',
                                ...
                                doc.save("Nexus_Prime_Executive_Report.pdf");
                            }}>
            ...
            {data && (
                <>
                    ...
                    {/* Split-Award Cart Optimization Banner */}
                    {comparison.split_award_optimization?.status === "RECOMMENDED" && (
                        <div style={{
                            ...
                            <div style={{
                                position: 'absolute', top: -20, right: -20, opacity: 0.05, fontSize: '120px'
                            }}>
                                <HiOutlineScissors />
                            </div>
                            ...
                            <div style={{ display: 'flex', alignItems: 'center', gap: '16px', marginBottom: '16px' }}>
                                <div style={{
                                    ...
                                    <HiOutlineScissors style={{ color: '#6c63ff' }} />
                                </div>
                                <div>
                                    <div style={{ fontSize: '12px', color: '#6c63ff', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '1px' }}>
                            ...
                            <p style={{ fontSize: '14px', color: 'var(--text-secondary)', marginBottom: '20px', maxWidth: '800px', lineHeight: 1.6 }}>
                                Nexus Prime algorithm identified an overlapping catalog between vendors. Wait, don't buy from just one!
                                By splitting your order across exactly <strong>{comparison.split_award_optimization.vendors_involved.length}</strong> vendors,
                                you achieve the absolute lowest total landed cost.
                                <br /><span style={{ color: '#00e676', fontWeight: 600 }}>This saves an additional ${comparison.split_award_optimization.potential_savings_usd.toLocaleString()} compared to buying from a single vendor.</span>
                            </p>

                            <div style={{ background: 'rgba(17,22,51,0.5)', borderRadius: 'var(--radius-md)', border: '1px solid var(--border-subtle)', overflow: 'hidden' }}>
                                <table className="data-table" style={{ margin: 0 }}>
                                    ...
                                    <tbody>
                                        {comparison.split_award_optimization.line_item_assignments.map((itm, idx) => (
                                            <tr key={idx}>
                                                <td>{itm.description}</td>
                                                <td><span style={{
                                                    background: 'rgba(108,99,255,0.15)', color: '#9fa8da',
                                                    padding: '2px 8px', borderRadius: '4px', fontSize: '12px', fontWeight: 600
                                                }}>{itm.best_vendor}</span></td>
                                                <td>{itm.quantity}</td>
                                                <td className="amount">${itm.unit_price.toLocaleString()}</td>
                                                <td className="amount font-bold text-white">${itm.subtotal.toLocaleString()}</td>
                                            </tr>
                                        ))}
                                    </tbody>
                                    <tfoot>
                                        <tr style={{ background: 'rgba(0,0,0,0.2)' }}>
                                            <td colSpan="4" style={{ textAlign: 'right', fontWeight: 600, color: 'var(--text-muted)' }}>Estimated Multi-Vendor Total:</td>
                                            <td className="amount" style={{ fontWeight: 700, color: '#00e676', fontSize: '16px' }}>
                                                ${comparison.split_award_optimization.estimated_split_total_usd.toLocaleString()}
                                            </td>
                                        </tr>
                                    </tfoot>
                                </table>
